...
%USER INPUTS
AWS = 'L1';            % Station name: L1,L2,L3
run_start = [2015,01,01];     % Model initiation time. L1:2015,L2:2019,L3:2019
run_end = [2015,12,31];       % Model termination time
...
demname = 'Data\L11dem.tif';%L1:L11dem.tif, L2:L21dem.tif, L3:L31dem.tif
filename = 'Data\lake_depth_L1.xlsx';% L1, L2, L3
sheetname = '2015';% L1:2015,L2:2019,L3:2016
...
for cci = 1:length(cc)
    ...
    % runoff loss rate and runoff
    cc_one = cc(cci);
    hydro_out_new = hydro_out.*(1-cc_one);
    % simulation begins
    s_tnum=190*24+14;% L2:158; L3:198
    for tt = s_tnum:212*24 %L2:170; L3:223
        ...
        % run lake module for every submerged column
        for i = 1:length(rown)
            m = rown(i);
            n = coln(i);
            ...
            if model_stage == 5
                ...
                lake_albedo(1) = 0.1911+exp(-1.0445.*lake_depth(1)-0.9183);
                %albedo multiplier
            ...
            if model_stage == 4 && ~isempty(lake_ind)
                lid_depth(m,n,tt) = length(1:lake_ind-1)*ice_grid_z+snow_z;
...
%         %
%         if ~any(ismember(unique(Model_stage(:,:,tt)), [3, 5]))
%             blake_area(tt,1) = length(find(squeeze(Model_stage(:,:,tt))==4))*cell_size^2;
%             blake_vol(tt,1) = sum(sum(squeeze(Lake_depth(:,:,tt)).*cell_size^2));
%         end
        % tt
        if mod(tt,24)==0
            disp(tt/24)
    ...
    lake_areas(:,cci) = lake_vol(doyz.*24-10)./1000000;
...
hold on;plot(INDEX(:,1),INDEX(:,2));plot(INDEX(:,1),INDEX(:,3))
