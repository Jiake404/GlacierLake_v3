function [runoff,melt,q,qm,SW_flux,T_s,T_b,track_a,snow_l_ro,snow_ro,snow_mwe] = runoff_prod_era5(SW_down_out, LW_down_out, air_T_out, rh_out, hum_out, pressure_out, wind_speed_out,...
...
for tt = 1:t_num-1
    ...
    if model_stage == 1
        ...
        %surface flux
         [q(tt),sens_flux(tt,1),lat_flux(tt,1),SW_flux(tt,1),LW_down(tt,1),LW_out(tt,1),CT(tt,1)]...
             = surface_flux1(SWD, ice_albedo, ta, WS, P, LWD, HUM, T(grid_num(tt)+1,tt), ice_albedo, 1, Io_ice, e_ice,0);

        if q(tt) > 0
           [qm(tt),sens_fluxm(tt,1),lat_fluxm(tt,1),SW_fluxm(tt,1),LW_downm(tt,1),LW_outm(tt,1)] = surface_flux1(SWD,...
               ice_albedo, ta, WS, P,LWD, HUM, T_melt, ice_albedo, 1, Io_ice, e_ice,0);
        end
    ...
    if model_stage == 2
        ...
        if snow_T(tt)>273.15&&air_T_out(tt)<270
            snow_T(tt)=air_T_out(tt);
            snow_e(tt) = snow_e(tt)+q_max(tt)+q(tt)*3600;
        end
